...
`check.py` 只能证明"代码没有语法错误"，证明不了"算得对"。
MD5 曾经对任意输入都返回同一个常量，而所有静态检查都是绿的。

```bash
python3 test.py          # 测中文站（dist/）
python3 test.py --en     # 测英文站（dist/en/）
```

它往构建产物里注入测试脚本、驱动真实 UI（点按钮、填输入框），
再用已知正确值比对输出，共 55 项，覆盖 9 个工具：

| 工具 | 覆盖内容 |
|------|----------|
| JSON | 格式化 / 压缩 / 校验（合法+非法）/ 排序 / 转义往返 / JSONPath / 转 JSON-CSV / 树视图 |
| 文本比对 | 新增行、删除行、完全相同时无差异 |
| 编码解码 | Base64 / URL / Unicode / HTML 实体 / Hex 各自往返 + 精确值 |
| 正则 | 匹配数量、分组捕获 |
| 时间戳 | 秒级、毫秒级、日期反向转换、实时时钟格式 |
| 哈希 | MD5 / SHA-1 / SHA-256 / SHA-512 / HMAC 的公开测试向量、AES 往返、UI 路径 |
| 代码格式化 | HTML / CSS / SQL 的格式化与压缩 |
| 字符串 | 大小写、去重、排序、UTF-8 字节统计 |
| 生成器 | UUID v4 格式与唯一性、密码长度、数字序列、随机数据条数 |

**注意**：测试必须等页面 init（以及 JSON 页那层 300ms 演示数据定时器）跑完再断言，
否则会出现"时好时坏"的假失败。JSON 页用 `__readyJson()` 轮询演示数据是否落地，
不依赖固定延时。
